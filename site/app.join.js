(function(){
  function qs(root, selArr){
    for (var i=0;i<selArr.length;i++){
      var el = root.querySelector(selArr[i]);
      if (el) return el;
    }
    return null;
  }
  ...
  async function submitJoin(ev){
    ...
    var out = document.getElementById('join-feedback');
    if (out) out.textContent = '';

    if (!window.FORM_ENDPOINT){
      if (out) out.textContent = 'Config error: FORM_ENDPOINT is not set';
      return;
    }
    if (!teamVal || !countryVal || !contactVal || !chVal || !plVal){
    ...
    try{
      ...
      if (out) out.textContent = 'Invalid FORM_ENDPOINT: ' + err.message;
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if (f){
      f.addEventListener('submit', submitJoin, false);
    }
  });
})();
