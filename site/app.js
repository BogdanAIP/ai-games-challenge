...
async function loadMiniBoard(){
  try{
    ...
    if(entries.length===0){
      document.getElementById('mini-board').innerHTML = '<p class="muted">No entries yet.</p>' + updated;
      return;
    }
    const rows = entries.map((e,i)=>(
      `<tr class="${i===0?'top1':''}">
         <td>${i+1}</td>
         <td>${e.title}</td>
         <td>${(e.youtube&&e.youtube.playlist)?`<a href="${e.youtube.playlist}" target="_blank" rel="noopener">Playlist</a>`:'—'}</td>
         <td style="text-align:right;"><strong>${(e.score??0).toFixed(3)}</strong></td>
       </tr>`
    )).join('');
    document.getElementById('mini-board').innerHTML =
      `<table><thead><tr><th>#</th><th>Game</th><th>Link</th><th style="text-align:right;">Score</th></tr></thead><tbody>${rows}</tbody></table>` + updated;
  }catch(err){
